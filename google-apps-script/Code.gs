 ...
 * Pairs with chandlerhillaagency-site → main.js POST JSON to /exec
 */

/**
 * Chandler Hill Agency — your contact & routing details (keep in sync with index.html)
 */
const CONFIG = {
  ...
  // Notification inboxes (all receive new-lead emails)
  EMAIL_RECIPIENTS: [
  ...
  // Direct lines (About → Direct Contact)
  PHONE_NUMBER: '(574) 309-0107',
  AGENCY_PHONE: '(574) 465-1495',

  // Office locations (Locations tab)
  OFFICES: [
    {
      name: 'South Bend',
      address: '2039 E Ireland Rd, South Bend, IN 46614',
      phone: '(574) 232-6945'
    },
    {
      name: 'Plymouth',
      address: '536 N Oak Dr, Plymouth, IN 46563',
      phone: '(574) 936-7157'
    },
    {
      name: 'Goshen',
      address: '114 E Clinton St, Goshen, IN 46528',
      phone: '(574) 534-5696'
    }
  ],
...
/** Column order — run setupSheetHeaders() once after first deploy */
const HEADERS = [
  'Lead ID',
  'Date',
  'Time',
...
/**
 * Run once from the Apps Script editor after deploy (or when adding columns to a blank tab).
 */
...
/** Global per-minute cap (health probes / burst abuse). */
...
function sendLeadNotification_(leadId, dateStr, timeStr, p) {
  ...
  const subject =
    'New Lead — ' + p.firstName + ' ' + p.lastName + ' (' + p.type + ') [' + leadId + ']';
